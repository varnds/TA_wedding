...
export function createSandTexture(baseHex) {
  const size = 512;
  const canvas = document.createElement("canvas");
  canvas.width = size;
  canvas.height = size;
  ...
  for (let i = 0; i < 12000; i += 1) {
    ...
    ctx.fillStyle = `rgba(${base.r + g},${base.g + g},${base.b + g},${0.08 + Math.random() * 0.12})`;
...
export function fabricMaterialProps(color, roughness = 0.84) {
  return { color, roughness, metalness: 0.01 };
}
