...
function GoldenHourLighting({ palette, seasonKey, isNight, lanternOn }) {
  ...
  if (isNight) {
    return (
      <>
        <ambientLight intensity={lanternOn ? 0.32 : 0.18} color="#2A3358" />
        <directionalLight
          position={[sx, sy + 4, sz + 4]}
          intensity={lanternOn ? 0.75 : 0.28}
          color="#8899CC"
          castShadow
          shadow-mapSize={[2048, 2048]}
        />
        <hemisphereLight args={["#3A4570", "#1A1F35", 0.35]} />
      </>
    );
  }
...
function SceneContents(props) {
  ...
  // Clear winter-day sky: a real blue up top fading to bright white at the horizon.
  const sky = isWinter
    ? { sky1: "#8FBAE8", sky2: "#BBD5F1", sky3: "#ECF4FC" }
    : { sky1: palette.sky1, sky2: palette.sky2, sky3: palette.sky3 };
  // Warm haze pulled in close so distant dunes dissolve into the air (the soft
  // atmospheric falloff that makes the reference feel dreamy and sun-bleached).
  const fogColor = isWinter ? "#EDF3FC" : palette.sky3;
  const fogRange = isWinter ? [17, 70] : [16, 66];
